...
class Randomizer {
	...
	static protected $logic_array = [
		,
	];
	...
	public function makeSeed(int $rng_seed = null) {
		...
		$randomized_order_locations = Filler::factory('Random')->shuffleLocations($locations->getEmptyLocations());
		...
		if ($this->config('rng_items')) {
			$this->fillItemsInLocations(array_fill(0, count($this->getItemPool()), Item::get('singleRNG')), $my_items, $randomized_order_locations);
		} else {
	...
	public function writePrizeShuffleToRom(Rom $rom) {
		...
		$prizes = [
			...
			0xE3, 0xE0, 0xDC, 0xE3, 0xE3, 0xE3, 0xE3, 0xDA,
			0xDA, 0xE3, 0xDA, 0xD8, 0xE3, 0xD8, 0xE3, 0xE3,
			0xE3, 0xE3, 0xE3, 0xE3, 0xDC, 0xDB, 0xE3, 0xDA,
			0xE0, 0xDC, 0xE3, 0xDE, 0xE3, 0xDB, 0xDD, 0xE3,
			0xE3, 0xD8, 0xE3, 0xDB, 0xDB, 0xE3, 0xE3, 0xD8,
		];
		...
		// write bonk prizes
		$bonk_items = array_merge(array_splice($shuffled, 0, 39), [
			0x79, 0x79, 0x79, 0x79, 0x79, 0x79, 0x79, 0x79, 0x79, 0x79, 0x79, // bees
			0xAC, 0xAC, 0xAC, 0xAC, 0xAC, 0xAC, 0xAC, 0xAC, 0xAC, 0xAC, 0xAC, 0xAC, // apples (need to fix door opening)
		]);
		$rom->setOverworldBonkPrizes(mt_shuffle($bonk_items));
