...
class Enemizer {
	...
	public function writeOptionsFile() {
		...
		$options = [
			"RandomizeEnemies" => (bool) ($this->settings['enemy'] ?? false),
			"RandomizeEnemiesType" => 3,
			"RandomizeBushEnemyChance" => true,
			"RandomizeEnemyHealthRange" => (bool) ($this->settings['enemy_health'] ?? false),
			"RandomizeEnemyHealthType" => (int) (($this->settings['enemy_health'] ?? 0) - 1),
			"OHKO" => false,
			"RandomizeEnemyDamage" => (bool) ($this->settings['enemy_damage'] != 'off'),
			"AllowEnemyZeroDamage" => true,
			"ShuffleEnemyDamageGroups" => in_array($this->settings['enemy_damage'], ['shuffle', 'chaos']),
			"EnemyDamageChaosMode" => (bool) ($this->settings['enemy_damage'] == 'chaos'),
			...
			"RandomizeDungeonPalettes" => (bool) ($this->settings['palette_shuffle'] ?? false),
			"SetBlackoutMode" => false,
			"RandomizeOverworldPalettes" => (bool) ($this->settings['palette_shuffle'] ?? false),
			"RandomizeSpritePalettes" => (bool) ($this->settings['palette_shuffle'] ?? false),
			...
			"RandomizePots" => (bool) ($this->settings['pot_shuffle'] ?? false),
			"ShuffleMusic" => false,
			"BootlegMagic" => true,
			"CustomBosses" => false,
			"AndyMode" => false,
			...
			"AllowKillableThief" => (bool) ($this->settings['enemy'] ?? false),
