...
class Sprite {
	...
	static public function all() {
		...
		static::$items = new SpriteCollection([
			...
			new Sprite("BeeGood", "Good bee again?", [0xB2]),
	...
	public function readPropertiesFromRom(Rom $rom) {
		...
		return [
			...
			'visibility' => $bytes[0] & 31,
			'hit_points' => $bytes[1],
			'damage_type' => $bytes[2] & 15,
			'unknown' => $bytes[2] >> 4,
			...
			'pallet' => $bytes[3] & 14,
			'unknown2' => $bytes[3] & 1,
			'ignore_collision' => $bytes[4] & 128,
			'statis' => $bytes[4] & 64,
			'persist' => $bytes[4] & 32,
			'hitbox' => $bytes[4] & 31,
			'hitbox_interaction' => $bytes[5] & 240,
			'unknown3' => $bytes[5] & 8,
			'unknown4' => $bytes[5] & 4,
			'death_boss' => $bytes[5] & 2,
			'falls_in_holes' => $bytes[5] & 1,
			'disable_interactions' => $bytes[6] & 128,
			'water' => $bytes[6] & 64,
			'shield_destruction' => $bytes[6] & 32,
			'damage_sound' => $bytes[6] & 16,
			'prize_pack' => $bytes[6] & 15,
			'death_conditional' => $bytes[7] & 128,
			'death_conditional2' => $bytes[7] & 64,
			'unused2' => $bytes[7] & 32,
			'deflect_missles' => $bytes[7] & 16,
			'collide_less' => $bytes[7] & 8,
			'impervious_sword_hammer' => $bytes[7] & 4,
			'impervious_arrows' => $bytes[7] & 2,
			'disabled' => $bytes[7] & 1,
		];
	}
