					...
					<table class="table table-striped table-sm">
						...
						<tbody>
							<tr v-for="(item, location) in value" class="spoil-item-location" :class="{ 'bg-info text-light': item == search.value
								...
								<td>{{ location }}</td>
								<td class="item">{{ $te('item["' + item + '"]') ? $t('item["' + item + '"]') : item }}</td>
							</tr>
						</tbody>
					</table>
				</tab>
					...
					<table class="table table-striped table-sm">
						...
						<tbody>
							<tr v-for="row in shops" class="spoil-item-location"
								...
								<td>{{ row.location }}</td>
								<td>{{ row.type }}</td>
								<td class="item">{{ row.item_0 ? $t('item.' + row.item_0.item) : '' }}<span v-if="row.item_0 && row.item_0.price"> ({{ row.item_0.price }})</span></td>
								<td class="item">{{ row.item_1 ? $t('item.' + row.item_1.item) : '' }}<span v-if="row.item_1 && row.item_1.price"> ({{ row.item_1.price }})</span></td>
								<td class="item">{{ row.item_2 ? $t('item.' + row.item_2.item) : '' }}<span v-if="row.item_2 && row.item_2.price"> ({{ row.item_2.price }})</span></td>
							</tr>
						</tbody>
					</table>
				</tab>
					...
					<table class="table table-striped table-sm">
						...
						<tbody>
							<tr v-for="row in playthrough" class="spoil-item-location" :class="{ 'bg-info text-light': row.item == search.value }">
								...
								<td>{{ row.location }}</td>
								<td class="item">{{ $te('item.' + row.item) ? $t('item.' + row.item) : row.item }}</td>
							</tr>
						</tbody>
					</table>
				</tab>
...
export default {
	...
	computed: {
		...
		items: (vm) => {
			let items = {};
			for (name in vm.rom.spoiler) {
				if (['meta', 'playthrough', 'Entrances', 'paths', 'Shops', 'Bosses'].indexOf(name) === -1) {
			...
			return Object.keys(items).sort().map((item) => {
				var nice_name = vm.$i18n.te('item.' + item) ? vm.$i18n.t('item.' + item) : item;
