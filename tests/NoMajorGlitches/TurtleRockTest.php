...
class TurtleRockTest extends TestCase {
	...
	public function testCanEnterWithEverything() {
		$this->assertTrue($this->world->getRegion('Turtle Rock')->canEnter($this->world->getLocations(), $this->allItems()));
	}
	...
	public function testEtherRequiredIfEtherIsEntryMedallion() {
		...
		$this->assertFalse($this->world->getRegion('Turtle Rock')->canEnter($this->world->getLocations(), $this->allItemsExcept(['Ether'])));
	}
	...
	public function testBombosRequiredIfBombosIsEntryMedallion() {
		...
		$this->assertFalse($this->world->getRegion('Turtle Rock')->canEnter($this->world->getLocations(), $this->allItemsExcept(['Bombos'])));
	}

	public function testQuakeRequiredIfQuakeIsEntryMedallion() {
		$this->world->getLocation("Turtle Rock Medallion")->setItem(Item::get('Quake'));

		$this->assertFalse($this->world->getRegion('Turtle Rock')->canEnter($this->world->getLocations(), $this->allItemsExcept(['Quake'])));
	}

	public function testCaneRequiredForEntry() {
		$this->assertFalse($this->world->getRegion('Turtle Rock')->canEnter($this->world->getLocations(), $this->allItemsExcept(['CaneOfSomaria'])));
	}

	public function testHammerRequiredForEntry() {
		$this->assertFalse($this->world->getRegion('Turtle Rock')->canEnter($this->world->getLocations(), $this->allItemsExcept(['Hammer'])));
	}

	public function testMoonPearlRequiredForEntry() {
		$this->assertFalse($this->world->getRegion('Turtle Rock')->canEnter($this->world->getLocations(), $this->allItemsExcept(['MoonPearl'])));
	}

	public function testMittRequiredForEntry() {
		$this->assertFalse($this->world->getRegion('Turtle Rock')->canEnter($this->world->getLocations(), $this->allItemsExcept(['TitansMitt'])));
	}

	public function testMirrorOrHookshotRequiredForEntry() {
		$this->assertFalse($this->world->getRegion('Turtle Rock')->canEnter($this->world->getLocations(), $this->allItemsExcept(['MagicMirror', 'Hookshot'])));
	}

	public function testNotOnlyMirrorRequiredForEntry() {
		$this->assertTrue($this->world->getRegion('Turtle Rock')->canEnter($this->world->getLocations(), $this->allItemsExcept(['MagicMirror'])));
	}

	public function testNotOnlyHookshotRequiredForEntry() {
		$this->assertTrue($this->world->getRegion('Turtle Rock')->canEnter($this->world->getLocations(), $this->allItemsExcept(['Hookshot'])));
	}
	...
	public function testBigKeyRoomDoesNotRequireFireRodIfKeyInCompassRoom() {
		$this->world->getLocation("[dungeon-D7-1F] Turtle Rock - compass room")->setItem(Item::get('Key'));
		$this->world->getLocation("[dungeon-D7-1F] Turtle Rock - Chain chomp room")->setItem(Item::get('Key'));
